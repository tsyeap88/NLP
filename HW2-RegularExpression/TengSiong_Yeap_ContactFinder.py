...
epatterns = []
#epatterns.append('([A-Za-z]+)@([A-Za-z]+)\.edu')               #1st attempt
#epatterns.append('([A-Za-z.]+)@([A-Za-z.]+)\.edu')             #2nd attempt
epatterns.append('([A-Za-z.]+)\s*@\s*([A-Za-z.]+)\.edu')        #3rd attempt
epatterns.append('([A-Za-z.]+)\s*@\s*([A-Za-z.]+)\.EDU')        #4th attempt cheriton uma case
epatterns.append('([A-Za-z.]+)<del>@([A-Za-z.]+)\.edu')         #5th attempt latombe case
#epatterns.append('[A-Za-z-]*@[-][A-Za-z-]*\.[-][A-Za-z-]*') #dlwh... output error like Manning
...
ppatterns = []
ppatterns.append('(\d{3})-(\d{3})-(\d{4})')               #1st attempt
ppatterns.append('\((\d{3})\)\s*(\d{3})-(\d{4})')         #2nd attempt #bgirod #ashishg
ppatterns.append('\[(\d{3})\]\s(\d{3})-(\d{4})')          #3nd attempt #nass
ppatterns.append('\+1\s(\d{3})\s?-?(\d{3})\s?-?(\d{4})')  #4th attempt #jurasky #shoham
